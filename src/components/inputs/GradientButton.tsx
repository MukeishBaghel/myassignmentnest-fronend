...
const GradientButton: React.FC<GradientButtonProps> = ({ children, bgClassName, className, ...props }) => {
    return (
        <button
            className={cn(`bg-primary_100 hover:bg-none h-12 group px-4 rounded-xl duration-300 ease-in transition-all text-primary shadow-btn_shadow hover:shadow-none hover:border border-primary-100 ${bgClassName}`)}
            {...props}
        >
